...
現在 Phase 2（空が出る）まで。キーボードで飛ばせて、物理ベースの大気散乱が入っている。
...
`?debug=1` を付けると速度、高度、迎角、G、バンク角、太陽高度の計器が出る。`?hour=18` で時刻（太陽高度）を、`?preset=low` で品質を切り替えられる。
...
`?capture=1&script=level&frame=240&hour=16` を付けて開くと決定論キャプチャモードになる。実時間を使わず指定フレームまで進めて 1 枚描き、止まる。太陽の位置も時刻から固定される。Playwright はこれを撮って基準画像と比べる。

## この先

Phase 3 でボリュメトリック雲と起伏のある地形。Phase 4 で機体モデル、Phase 5 で武装と HUD。Phase 6 で敵 AI、Phase 7 でミッション。
