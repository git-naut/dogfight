...
理由は検証の土台にある。段 9 から段 14 の前半までは「固定の入力を両側へ渡して
数で突き合わせる」形で移植を確かめてきた。高さ場は `src/sim/terrain.ts` の
`heightAt` と 1 cm 以内で一致することを 128 点で測れる。

`terrain.frag` の出力はそうならない。パッチの選択、インスタンス属性、法線
テクスチャ、機体の影マップ、雲影マップ、大気の放射輝度がそろって初めて
値が決まる。単独のプローブに切り出せる部分がほとんど残らない。

地形の場面を node 経路へ先に立てる道も測った上で採らなかった。段 17 が
`RenderPipeline` と `pass(scene, camera)` でパイプラインの形を変えるので、
**ここで組む場面は段 17 で作り直すことになる。**

`getSplitIlluminance` の置き換えは絵が変わる変更で、これは段を分ける理由が
はっきりしている唯一のものだった。**差分の帰属を切り分けられる状態、つまり
ほかの移植が全部終わってから入れる。**段 18 の関門は「理由の付かない差分が
1 枚でも残っている間は撮り直さない」なので、検証できない移植を先に積むと
そこで払うことになる。

段 14 で入れたモーフの基準位置の uniform 化は、絵を動かさないので前倒しの
...
## 既定を旧経路へ戻した（2026-09-11 追記、段 20c）

段 20b で `DEFAULT_BACKEND` を `node` にしたが、**ライブ飛行中に描画ループ
ごと止まる**ことが分かったので `webgl` へ戻した。公開ページで実際に踏んだ。

品質の自動降格が引き金で、`clouds.setQuality` が組み直したマーチの材質を
最初に描く `createBindGroup` が `undefined.mipLevelCount` で落ちる。降格を
止めれば 4,148 フレーム回り、許せば約 450 フレームで死ぬ。切り分けは 9 手
かけて外側を全部潰した（`docs/measuring.md` の段 20c）。材質をいつ作るかでも、
何を共有するかでも、古いほうをどう捨てるかでもない。

### 切り替え直す前に要るもの

**ライブループを数百フレーム回して降格まで通す検査。**キャプチャモードは
N ステップ進めて 1 枚描いて止まるので、この経路を一度も通らない。E2E
277 件が全部緑のまま公開まで抜けた理由がそれ。

node の基準画像 42 枚（`*-chromium-webgpu-linux.png`）は残してある。直して
切り替え直すときにそのまま使う。
