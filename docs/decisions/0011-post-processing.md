...
炎だけが MRT の `emissive` を書き、ブルームの閾値の外で足す形にした。プリセット表の `bloomEmissive`（ブルームを掛ける medium 以上で true）、倍率は `EMISSIVE_BLOOM_GAIN = 4`。

**最初はブルームの入力へ倍率を掛けて足し、崖になった。**炎の `emissive` は 0.03 前後で、閾値（露出前 1.33）を越えるまで何も起きない。`low-pass-afternoon` で倍率 40 までは外周 +0.0%、60 で +106.2%。スロットルが少し動くだけで光ったり消えたりする。`BloomNode.highPassFn` を差し替え、合成した絵には three と同じ閾値を、発光体には閾値を通さず倍率だけを掛けた。
...
外周の箱は F/A-18E の炎へ置き直した（`tools/bloom-sweep.mjs` の `REGIONS.glow` と `GLOW_CORE`）。C 型のときの箱は E 型では炎の尾より下の海面を測っていた。アフターバーナーの出ていない `level-backlit` と `bank-left-dusk` は 3 領域とも ±0.0%。

炎は three の `MeshBasicMaterial` なので、`mrtNode` を差すために node 版へ先に写す（`nodeFlameMaterial.ts`、機体の材質と同じ `fromMaterial`）。GLSL 経路は作り手を渡さないので変わらない。

**閾値の関数の差し替えは単体テストで守れない。**GPU の上でしか値が出ないので、判定道具の数値と基準画像が見張る。

基準画像は 42 枚のうち 13 枚が動いた。アフターバーナーが出ている構図（低空通過、上昇、翼端渦、地形の上、カタパルト、ミッション）だけで、8 階調を超える差の外接はどれも機体と炎のまわり（x 444〜835）。カタパルトとミッションは滲みが甲板へ広がって画面の下端まで届く。HUD は 2D の canvas なので動かない。残りの 29 枚は炎が出ていない。

実機（Intel Xe-2LPG、2400x1183、high、20 秒）で発光体なし 10.0 / 10.4 ms、あり 10.7 / 10.4 ms。差は同じ条件どうしの振れ（0.4 ms）と同じくらいで、60 fps、降格なし。ライブでは炎が出ていないことが多いので、主に MRT の 2 本目を書く費用を測っている。
