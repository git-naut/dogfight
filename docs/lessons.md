...
| `RenderTarget3D` へ層ごとに焼いて読み戻すと、1,024 バイトすべてが 0 で返る。同じ層を 2D のレンダーターゲットへ焼いたものは GLSL 版とビット一致するので、シェーダではなく層の扱いが効いていない。WebGL2 でも WebGPU でも同じ | 段 12 で雲の密度に要る体積を焼こうとしたとき | 未対応。段 12 の前段で止まっている。切り分けは `docs/measuring.md` に残した |
