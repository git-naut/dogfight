 ...
 *
 * Bayer の秩序ディザなら 4x4 の周期で均等に散り、隣接画素の値が偏らない。
 * 時間方向の蓄積を持たない構成ではこちらが素直に見える。
 */
...
float dither(ivec2 pixel) {
  int index = (pixel.y & 3) * 4 + (pixel.x & 3);
  return BAYER_4X4[index] * (1.0 / 16.0);
}
...
void main() {
  ...
  // ディザの振れ幅を抑える。1/4 解像度で描いて 4 倍に拡大するので、
  // 隣り合う画素の開始位置が離れるほど格子模様として目立つ
  float t = start + baseStep * offset * 0.6;
