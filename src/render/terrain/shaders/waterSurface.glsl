// 海面の色。海面の断片シェーダと突き合わせのプローブが同じ本体を読む。
//
// 理由と作りは `terrainSurface.glsl` と同じ。**写しを 2 つ持たない。**
// 枝は `out` で出す。海面は枝が 4 つある（1 タップの粗い深さ、双三次の
// 深さ、白波、波の法線）ので、絵だけでは通ったかどうかが分からない。
 ...
 *
 * **`active` は GLSL の予約語。**引数名に使ってコンパイルが落ちた。
 * `terrain.frag` が `patch` と `half` で同じ失敗を記録している
 */
vec3 waterWaveNormal(vec2 world, float toCamera, float waveTime, out float applied) {
 ...
 * 海面の色。
 *
 * @param world         ワールド座標。y は 0（平らな板）
 * @param cameraPos     カメラのワールド位置
 * @param waveTime      sim のフレーム番号から導いた秒。実時間を渡さないこと
 * @param useSpecular   太陽のスペキュラを乗せるか
 * @param aircraftShade 機体の影の明るさ 0..1。1 なら日向
 * @param branches      通った枝。R が 1 タップ、G が双三次、B が白波、A が波
 */
vec3 waterSurfaceColor(
  ...
  float waveApplied;
  vec3 normal = waterWaveNormal(world.xz, toCamera, waveTime, waveApplied);
  branches = vec4(0.0, 0.0, 0.0, waveApplied);
  ...
  float coarse = terrainHeightNearest(world.xz);
  float depth;
  if (coarse < -(SHALLOW_DEPTH + 160.0)) {
    depth = -coarse;
    branches.r = 1.0;
  } else {
    depth = max(-terrainHeight(world.xz), 0.0);
    branches.g = 1.0;
  }
  ...
  if (foam > 0.0) {
    branches.b = 1.0;
    float band = 0.5 + 0.5 * sin(dot(world.xz, vec2(0.86, 0.51)) / 23.0 + waveTime * 1.7);
