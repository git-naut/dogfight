 ...
 * 搭載するミサイルの数。
 *
 * F/A-18C のモデルには Sidewinder が 6 発付いていた。同時に飛ばせる数と
 * 搭載数は別だが、Phase 5 では区別しないでプールも同じ数にした。
 *
 ...
 * 外れを見込んで 8 発に置く。
 *
 * **推定だが、実機で確かめた。**段 14（2026-08-31）に通しで遊び、この数で
 * 足りると判断した。数値の裏付けではなく人の手ごたえによる確認。
 */
