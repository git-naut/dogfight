 ...
 *
 * 値は F/A-18E の公表値を基準にした。戦闘重量 21,320 kg、主翼面積 46.45 m²、
 * 翼幅 13.62 m、F414-GE-400 × 2 のアフターバーナー推力 195.72 kN。
 * 導出と根拠は docs/flight-model.md に書いてある。
 *
 * Phase 1 では F-16、Phase 4 から Phase 8 までは F/A-18C を基準にしていた。
 * 見えているものと物理を揃える方針は変えていない。機体モデルを E 型へ
 * 差し替えたので、諸元も E 型で取り直した。
 *
 * **C 型から重くなったぶん、翼と推力も大きくなっている。**翼面荷重は
 * 4,395 → 4,503 N/m²、推力重量比は 0.96 → 0.94 で、どちらも公表値
 * （459 kg/m²、0.93）と一致する。E 型が C 型よりわずかに鈍いのは実機どおり。
 */

const DEG = Math.PI / 180

const WING_SPAN = 13.62
/** 500 ft² の換算値。公表の m² 表記は 46.5 で、丸めの差は 0.1% */
const WING_AREA = 46.45

export const AIRCRAFT = {
  /** 質量 kg。47,000 lb（fighter escort 形態の gross weight） */
  mass: 21_320,
  /** 主翼面積 m² */
  wingArea: WING_AREA,
  /** アスペクト比 = 翼幅² / 翼面積 */
  ...
  /** 海面高度・アフターバーナー時の最大推力 N。F414-GE-400 が 22,000 lbf × 2 */
  maxThrust: 195_720,

  /**
   * 揚力傾斜 /rad。
   * 薄翼理論の 2π にアスペクト比 3.99 の有限翼補正を掛けた値。
   */
  liftSlope: 4.19,
  /**
   * 揚力係数が頭打ちになる迎角 rad。
   ...
   * 26.0 度で CLmax が 1.90 になり、LEX を持つ戦闘機の妥当な範囲に収まる。
   *
   * この平坦部が F/A-18 の高迎角性能の表現。Hornet が 35 度まで引けるのは
   * CLmax が高いからではなく、揚力が頭打ちになったあとも操縦できるため。
   * 平坦部を作らずに失速角を 38 度まで伸ばすと CLmax が 2.78 になり、
   * 実機の 1.8 前後から外れてしまう。
   *
   * **C 型は 27.2 度だった。**翼が大きくなって揚力傾斜が 4.0 から 4.19 へ
   * 上がったぶん、同じ CLmax に届く迎角が浅くなる。平坦部は 1.2 度広がる
   */
  clPeakAngle: 26.0 * DEG,
   ...
   * 有害抗力係数。
   *
   * クリーン形態の F/A-18 は 0.024 前後。モデルは翼端ミサイルとパイロンを
   * 付けた形態なので少し上げる。
   *
   * **E 型でも据え置く。**胴体が太くなったぶん抗力は増えるが、割る翼面積も
   * 25% 増えている。係数として公表された値が無いので動かす根拠がない
   */
  cd0: 0.026,

  /** 構造の G 制限。SAR の design load factor が 7.5 g */
  gLimit: 7.5,
  /**
   * フライバイワイヤの迎角制限 rad。
   *
   * Hornet の制御は 35 度まで許す。上の平坦部（26.0 度から 38 度）の内側に
  ...
  maxPitchRate: 0.6,
  /**
   * 毎秒 229 度。**E 型の公表値が無いので C 型のまま据え置く。**
   * 翼が大きくなったぶん実機では鈍るはずだが、数を当てる根拠がない
   */
  maxRollRate: 4.0,
  maxYawRate: 0.35,
   ...
   * 6.0 × ln(0.456 / 0.25) = 3.6 秒。254 m/s なら 910 m ぶん伸びる。
   *
   * 最初は 3 秒（1.8 秒ぶん）にしたが、渦の出る距離をもっと長くという
   * 指摘を受けて倍にした。
   */
  vaporFallTau: 6,
