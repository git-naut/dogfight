...
export const SCRIPTS = {
  ...
  'mission-01': {
    ...
    seed: 20260828,
    /**
     * **敵の位置はここからの相対。**自機は空母の甲板へ移されるが
     * （`World` が `launch` を見て移す）、敵は移す前の位置を基準にする。
     * 先に甲板（海面から 20 m）へ移すと敵が海面すれすれに並ぶ。
     *
     * 高度を 3,000 m から 1,200 m へ下げた。**射出は海面から始まる。**
     * 3,000 m のままだと最初の接敵まで上昇に時間がかかりすぎる。
     * 速度も巡航の 250 m/s に置いたままだが、これは敵の初期速度に使われる
     * だけで自機には効かない（甲板では 0）。
     */
    spawn: { altitude: 1200, speed: 250 },
    /**
     * 空母。艦首を −Z（主峰の島の方向）へ向ける。
     *
     * 島は `(1500, -11000)` にある（`terrain.ts`）。射出直後に正面へ
     * 見えるので、上昇しながらの目印になる。
     *
     * **距離は上昇の余裕で決めた。**敵は原点付近（自機の spawn 位置からの
     * 相対）にいる。空母を Z 2,000 に置くと射出の 9.1 秒後に落とされ、
     * 高度 180 m しか稼げない。Z 8,000 なら 21.4 秒あって 1,147 m まで
     * 上がれる（実測。機首を引くだけの自動操縦で測った）。
     *
     * **勝てるかどうかは機械では測れない。**測定用の自動操縦はどの距離でも
     * 5 機とも残したまま落とされる（段 6 と同じ）。段 14（2026-08-31）で
     * 実機で確かめ、この配置でよいと判断した。
     */
     ...
     * 詰める時間が要る。**探す時間は測れていない。**倍を見て 300 秒に置いた。
     *
     * 測定用の自動操縦が勝てないので機械では決められない
     * （`tests/sim/mission.test.ts`）。**段 14（2026-08-31）に実機で確かめ、
     * この長さでよいと判断した。**数値の裏付けではなく人の手ごたえによる確認。
     */
    missionSeconds: 300,
    enemies: [
    ...
    /**
     * **射出を始めるためのキーフレーム。**キャプチャモードとテストは入力
   ...
   * 空母を見るための台本。
   *
   * **空母を出す台本を絞ってある。**基準画像 42 枚のうち 38 枚は空母の無い
   * 海を撮ってあるので、既存の台本に置くとその 38 枚が差分を出す。空母を
   * 持つのは `carrier-deck` と `catapult-launch` と `mission-01` の 3 本。
   *
   * **自機は必ず原点に湧く**（`SpawnSpec` は高度と速度しか持たない）ので、
   * 空母のほうをずらす。高度 220 m、700 m 先。
   *
   * 位置は目で合わせず計算した。速度 120 m/s での画角は縦 61.5 度
   * （`camera.ts` の `FOV_BASE` から速度で開く）。カメラは自機の後方
   * 23 m・上 6.8 m で、注視点は機首の 60 m 先。この配置だと空母までが
   * 753 m、画面中心から下へ 11.3 度、船体 334 m の見かけが 25.0 度で、
   * 縦の端（30.8 度）の内側に収まる。
   *
   * 目で合わせた最初の 2 案はどちらも外れた。700 m は船が小さすぎて甲板の
   * 標識が読めず、420 m は見かけが 39.5 度になって艦尾が画面から出た。
   * **基準画像は差分が読めなければ意味がない。**
   *
   * 艦首を少し右へ振ってあるのは、真正面から見ると斜め甲板と艦橋が
   * 重なって奥行きが読めないため。段 11 の降着装置と段 12 の射出は
   * ここを起点にする。
   */
  'carrier-deck': {
    name: 'carrier-deck',
    seed: 20260831,
    spawn: { altitude: 220, speed: 120 },
    carrier: { x: 40, z: -700, heading: 0.35 },
    keyframes: [],
  },
  /**
   * 降着装置を見るための台本。
