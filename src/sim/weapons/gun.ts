 ...
 * | 初速 | 1,030 m/s | 公表値 |
 * | 携行弾数 | 578 発 | 公表値。**ゲームでは 1,800 発**（`MAGAZINE` の注記） |
 ...
 * 携行弾数。**ゲームの値で、公表値ではない。**
 *
 * M61A1 の公表値は 578 発。Phase 6.5 までそれを使っていたが、ミッション 01
 * （敵 5 機撃墜）に届かない。
 *
 * 実測が 2 つあり、値が大きく違う（`docs/weapons.md`）。
 *
 * | 条件 | 1 機あたりの消費 |
 * | `gun-pass`（理想的な後上方から撃つ台本） | 95 発（命中 60、63%） |
 * | 1 対 1 の実戦（後方 1,800 m から交戦） | **545 発**（命中 60、11%） |
 *
 * **差は命中率。**理想の構図では撃った弾がほぼ当たるが、旋回しながらの
 * 撃ち合いでは 9 割が外れる。ミッションで効くのは後者。
 *
 * ミッション 01 は敵 5 機。**全機を機銃で削る前提は過大**で、ミサイルでも
 * 落とす。機銃 3 機・ミサイル 2 機と見て 545 × 3 = 1,635 発。外れを見込んで
 * **1,800 発に置く。**公表値の 3.1 倍で、18 秒ぶんの連射にあたる。
 *
 * **推定だが、実機で確かめた。**8 機戦の測定用に書いた自動操縦は
 * 「人間の下限の腕」として成立しない（撃墜 0 で自滅する。
 * `tests/sim/mission.test.ts`）ので、機械では決められなかった。
 * 段 14（2026-08-31）に通しで遊び、この数で足りると判断した。
 * 数値の裏付けではなく人の手ごたえによる確認。
 */
export const MAGAZINE = 1800
/**
 * 敵機の携行弾数。**自機とは別に持つ。**
 *
 * `MAGAZINE` を 1,800 発へ増やしたとき、`new Gun(ENEMY_BULLET_POOL)` が
 * 第 2 引数を省いていたので敵も 3.1 倍になった。**自機の弾薬を増やす意図に
 * 敵の強化は含まれていない。**公表値の 578 発に据え置く。
 *
 * 敵はバーストで撃つ（0.6 秒撃って 1.2 秒休む、実効 33 発/秒）ので、
 * 578 発は 17 秒ぶんにあたる（`tests/sim/enemyFire.test.ts`）。
 */
export const ENEMY_MAGAZINE = 578
