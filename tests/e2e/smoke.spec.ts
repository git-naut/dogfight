...
test.describe('スクリーンショット回帰', () => {
  const scenes = [
    ...
    // 実測でフレアの寄与は 1,585 画素・最大 109 階調（`?flares=0` との引き算）。
    // **赤い燃焼が出ていることをこの 1 枚で見張る。**最も赤い画素が
    // `rgb(207,118,86)`（赤み 89・彩度 121）。加算合成に戻すか、芯の深度書きを
    // 落とすと、ここが赤み 8〜45 まで下がる（`docs/decisions/0008` の表）
    ...
    // 実測で寄与は 149 画素・最大 180 階調（定常の 109 より強い）。
    // 白熱なので彩度は 14 しかない。閃光を落とすとここが赤くなる
