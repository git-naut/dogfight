...
import { SCENES, captureParams, type CaptureQuery } from './scenes.mjs'
...
test.describe('スクリーンショット回帰', () => {
  // 構図は `tests/e2e/scenes.mjs` が正本。`tools/exact.mjs` も同じ配列を読む
  const scenes = SCENES
 ...
 * ない。基準画像 42 枚は 1 画素も動かない（`tools/exact.mjs` で確認済み）。
 */
