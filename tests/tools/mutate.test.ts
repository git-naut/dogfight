...
describe('docs/lessons.md', () => {
  ...
  it('穴の数を数えられる', () => {
    ...
    // なる前に洗う。**
    // 段 20c で 10 へ。**既定を node へ切り替えたら、遊んでいると約 450
    // フレームで描画ループごと止まった。**E2E 277 件は全部緑のまま公開まで
    // 抜けた。キャプチャモードは 1 枚描いて止まるので、ライブループを
    // 数百フレーム回して降格まで通す経路を検査が持っていない。既定は
    // 旧経路へ戻した（`DEFAULT_BACKEND`）。**この検査を作るまで切り替え
    // 直さない。**
    //
    // 段 20c の決着で 11 へ。**報告された症状を報告された操作で再現する
    // まで数値を読まない**が守れていない。「傾けると雲も同じ動きをする」の
    // 報告に対し、再現しないまま 3 回ぶん数字を出した（測る領域に雲も
    // 地形も入っていなかった）。**これは機械で縛れない。**測る前に絵を
    // 見る、という人の判断しかない。
    //
    // 段 20c の内訳は 9 → 11 で +2。足したのは「プリセット表に列が
    // あるのに実装が読まない」と、この「再現するまで数値を読まない」。
    // **ライブループの穴は同じ段で開けて同じ段で塞いだ**（4 段切り替えの
    // 検査）ので、差し引きに残っていない。
    //
    // 2026-09-13 に 12 へ。**同じ日に 1 つ埋めて 1 つ開けた。**
    //
    // 埋めたのは「E2E が `TestHook` の 264 行の写しを持っている」。
    // `import type` で本家を読む形にして写しを捨てた。
    //
    // 開けたのは「実機（`node-webgpu`）でだけ出る雲の追従を、手元で再現
    // できていない」。**報告は 2 度目。**段 20c で「描画停止の症状」と
    // 結論したが、three 0.186 で描画停止の根本が直った後も再現したので
    // 独立した欠陥だと分かった。**測り方が 3 度とも無効だった**（上半分を
    ...
    // 配分はライブループの引き算で取っている。
    //
    // 2026-09-18 に 14 へ。**`cmd | tail && commit` が cmd の失敗を飲む。**
    // パイプの終了コードは最後の段のものになるので、型エラーが出ていても
    // `&&` が成立してコミットが通った。「`npm run build` の出力を捨てない」
    // 「`exit code 0` は成功を意味しない」と**同じ罠の 3 度目**で、形を変えて
    // 出るので機械で縛れていない。**2026-09-19 に 4 度目を踏んだ。**今度は
    // `npm test ... | head -3; git add && git commit` で、`;` が失敗を飲んだ。
    // 落ちた 1 件に気づかないまま push まで通した。
    //
    // 2026-09-19 に 15 へ。**機体の差し替えでドローコールが 3.4 倍になった。**
    // `ac3d-to-glb.mjs` はマテリアルごとにメッシュを統合するが、
    // `f18e-to-glb.mjs` は頂点を触らない方針なので 220 部品がそのまま出る。
    // 実機は 60 fps を保っていて（GPU はむしろ下がった）、標的機が 8 機
    // そろう構図もいまの台本には無い。**統合するには `scene.bin` の
    // インターリーブを並べ直すことになる**ので着手していない。
    //
    // **減らしたときは下げる。増やすときは理由を書く。**
    expect(holes).toBeLessThanOrEqual(15)
  })
})
