...
`,
  ...
  },
  {
    ...
    find: '      testIgnore: [/node-fallback\\.spec\\.ts/, ...MUTATE_ONLY, ...NODE_PATH_DUPLICATE],',
    replace: '      testIgnore: [/node-fallback\\.spec\\.ts/, ...NODE_PATH_DUPLICATE],',
    expect: 'tests/tools/e2eConfig.test.ts',
    lesson: 'project の `testIgnore` は上位の `testIgnore` を上書きする',
  },
  {
    id: 'node-path-run-twice',
    kind: '文の削除',
    file: 'playwright.config.ts',
    find: '      testIgnore: [/node-fallback\\.spec\\.ts/, ...MUTATE_ONLY, ...NODE_PATH_DUPLICATE],',
    replace: '      testIgnore: [/node-fallback\\.spec\\.ts/, ...MUTATE_ONLY],',
    expect: 'tests/tools/e2eConfig.test.ts',
    why: '既定が node のあいだ、主の project と chromium-node は同じ引数・同じ宣言。両方で回すと 25 本を 2 回流す（CPU の 7.1%）',
  },
  {
    id: 'scene-pass-no-mrt',
    kind: '文の削除',
    file: 'src/render/pipeline/nodeOutput.ts',
    find: "        ...(normals ? { normal: directionToColor(normalView) } : {}),\n",
    replace: '',
    expect: 'tests/render/scenePass.test.ts',
    why: 'SSR が読む法線の面が作られない。getTextureNode だけでは 2 本目の出力を書き出さない',
  },
  {
    id: 'terrain-fragment-node',
    kind: '条件の固定',
    file: 'src/render/terrain/nodeMaterials.ts',
    find: '  ;(material as unknown as { outputNode: unknown }).outputNode =\n    terrainSurfaceNode(',
    replace: '  ;(material as unknown as { fragmentNode: unknown }).fragmentNode =\n    terrainSurfaceNode(',
    expect: 'tests/render/surfaceMrt.test.ts',
    lesson: '`fragmentNode` を持つ材質には MRT が当たらない',
  },
  {
    id: 'flame-mrt-missing',
    kind: '文の削除',
    file: 'src/render/pipeline/nodeFlameMaterial.ts',
    find: '    ;(copy as THREE.Material & { mrtNode: unknown }).mrtNode = mrt({ emissive: output })\n',
    replace: '',
    expect: 'tests/render/nodeFlameMaterial.test.ts',
    why: '炎が emissive を書かないと、場面の既定の 0 のままで選択的ブルームが何も拾わない。例外もコンソールも出ない',
  },
  {
    id: 'afterburner-ignore-factory',
    kind: '条件の固定',
    file: 'src/render/aircraft/afterburner.ts',
    find: '  const innerMaterial = material(\n',
    replace: '  const innerMaterial = keepFlameMaterial(\n',
    expect: 'tests/render/nodeFlameMaterial.test.ts',
