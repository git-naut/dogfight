...
`,
  ...
  },
  {
    ...
    find: "near(p.x, 1.43, 0.5) && near(p.absZ, 5.41, 0.5) && p.z < 0",
    replace: "near(p.x, 1.43, 2.0) && near(p.absZ, 5.41, 2.0) && p.z < 0",
    expect: 'tests/tools/f18eParts.test.ts',
    why: '舵面を名前ではなく位置で当てているので、閾値が広いと隣の部品を巻き込む。実測で 4 倍に広げると 1 件が 6 件になる',
  },
  {
