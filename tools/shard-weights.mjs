...
//
// **この重みは 1 回当てたら取り直さない。**12 分割のレポートで `--shards 16`
// を計算して当てたら予測 872〜916 秒に対し実測 441〜1,578 秒。16 分割の実測
// から取り直しても予測 745〜854 秒に対し実測 378〜1,500 秒。**同じ分割数から
// 計算しても外れる。**
//
// 理由は 2 つの run を並べると出る。総テスト秒は ±4% で安定するのに、**1 本の
// 最長が 201.7 から 342.1 秒へ 1.7 倍振れる。**どのテストが重くなるかが実行
// ごとに変わるので、前回の区間が次回は別の合計になる（SwiftShader は CPU
// 律速なので競合がそのまま所要へ出る）。**平均では釣り合うが最長は運。**
//
// 予測式そのものは合っている。最大テスト秒 1,578 秒に対する予測 809 秒に対して
// 実測 846 秒。**外れるのは入力のほう。**余裕が足りないときは重みではなく
// 分割数で薄める。
 ...
 *
 * **検算はレポートを撮ったときの並びで行う。**観測したシャード番号と
 * 突き合わせたあとで除く。除かずに釣り合わせると、抜ける区間のぶんだけ台が
 * 軽く見える。
 *
 * 撮ったときの並びは**呼ぶ側が作る。**逆テストが毎回走っていたころの
 * レポート（2026-09-25 より前）なら `MUTATE=1` を付けて走らせる。設定を
 * 変えて並びから外すときは、変える前の設定のまま走らせる
 */
